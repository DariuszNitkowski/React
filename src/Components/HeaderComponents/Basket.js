...
const Basket = (props) => {
    ...
    const showSingleProduct=(id)=>{
        axios
        .get(`http://localhost:5000/product/findbyId/${id}`)
        .then((res=>{
            props.passData({from: "basket", data: res.data, to: "singleproduct"})
            props.history.push({pathname:"/singleproduct"})
        }))
        .catch(()=>setMessage("Cant load the product"))
    }

    const handleDelte=(idDel, index)=>{
    ...
    return ( <>
            ...
            <table id="basketTable">
                ...
                <button onClick={()=>handleDelte(item._id, index)}>Delete</button></td>
